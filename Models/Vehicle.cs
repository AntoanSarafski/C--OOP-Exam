...
namespace EDriveRent.Models
{
    public abstract class Vehicle : IVehicle
    {
        ...
        public virtual void Drive(double mileage)
        {
            ...
            BatteryLevel = (int)Math.Round(BatteryLevel * (mileage / MaxMileage)); 
        }
        ...
        {
            string isDamagedAsString;
            if (IsDamaged)
            {
                isDamagedAsString = "damaged";
            }
            else
            {
                isDamagedAsString = "OK";
            }
            return $"{Brand} {Model} License plate: {LicensePlateNumber} Battery: {BatteryLevel}% Status: {isDamagedAsString}";
        }
    }
}
